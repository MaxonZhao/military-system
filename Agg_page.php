...
<body>
    ...
    <form method="POST" action="Agg_page.php">
    ...
    function handleAgg() {
        $result = executePlainSQL("SELECT Count(*) FROM Combatant WHERE Age > {$_POST['aggNum']}");

        if (($row = oci_fetch_row($result)) != false) {
            echo "<br> There are ". $row[0] ." people/person older than {$_POST['aggNum']} in Combatant. <br>";
        }
    } 
    if (isset($_POST['clickAgg'])) {
        if (connectToDB()) {
            handleAgg();
            disconnectFromDB();
        }
